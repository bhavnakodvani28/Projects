...
<?php 
         if(isset($_SESSION['user']))
            {
               header("refresh:5;url=member_profile.php");
            }
            if(isset($_SESSION['prdids']))
            {
               $cn=mysqli_connect("localhost","root","","focusdb") or die ("check connection");
                              
               foreach($_SESSION['prdids'] as $selected)
               {
                  $sql="insert into producttransaction (prid,uid) VALUES($selected,$_SESSION[userid])";               
            ...
            if(isset($_GET['pkid']))
            {
               $cn=mysqli_connect("localhost","root","","focusdb") or die ("check connection");
//               $result= mysqli_fetch_assoc(mysqli_query($cn,$sql));               
               $pkm=mysqli_fetch_array(mysqli_query($cn, "select pkmonth from packagedetail where pkid=$_GET[pkid]"));
               $pmon = $pkm['pkmonth'];
               $stdate = date('Y-m-d');
               $enddate = date('Y-m-d', strtotime("+".$pmon." month"));
               
               mysqli_query($cn, "INSERT INTO packagetransaction (pkid, uid, startdate, enddate) VALUES ($_GET[pkid], $_SESSION[userid], '$stdate', '$enddate')");
               echo "INSERT INTO packagetransaction (pkid, uid, startdate, enddate) VALUES ($_GET[pkid], $_SESSION[userid], '$stdate', '$enddate')";
            }
         ?>
<!DOCTYPE html>
<html lang="zxx">
...
<body>
      ...
      <section class="about-inner py-lg-4 py-md-3 py-sm-3 py-3">
		 ...
		 <!---728x90--->
			...
			<h1>You are now being redirected to payment gateway...
            <i class="fa fa-cog fa-spin fa-3x fa-fw"></i><br>
            <i class="fab fa-amazon-pay"></i>
<span class="sr-only">Loading...</span><br><br>Thank you for visiting!!</h1>
			</div>
